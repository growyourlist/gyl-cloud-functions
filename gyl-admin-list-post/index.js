...
exports.handler = async (event) => {
	try {
		...
		const currentRes = await db.get({
			TableName: `${dbTablePrefix}Settings`,
			Key: { settingName: 'lists'},
		})
		const settingExists = !!currentRes.Item
		const settingValueExists = settingExists && Array.isArray(currentRes.Item.value)
		const listSetting = settingValueExists ? currentRes.Item : {
			settingName: 'lists',
			value: [],
		}
		const currentIndex = listSetting.value.findIndex(i => i.id === listData.id)
		const newList = currentIndex >= 0 ? Object.assign(
			{},
			listSetting.value[currentIndex],
			{
				name: listData.name,
